"""Meridium extra features: puzzles, ratings, unlocks, terminal, daily signal, custom coaches."""
...
_THEME = {
    "Gotham Chess": ("#e8e0ff", "#5b4a9e"),
    "Hikaru": ("#dbeafe", "#1e3a5f"),
    "Magnus": ("#f5f0e6", "#4a5568"),
    "Anna Cramling": ("#fce7f3", "#9d174d"),
    "Botez": ("#ffe4e6", "#9f1239"),
    "Fabi": ("#e0f2fe", "#0c4a6e"),
    "Naroditsky": ("#cffafe", "#0e7490"),
    "Eric Rosen": ("#ffedd5", "#9a3412"),
}

def apply_extra_to_html(html, coach_name="Soju"):
    light, dark = _THEME.get(coach_name or "Soju", ("#eeeed2", "#769656"))
    css = (
        "<style id='mer-theme'>.sq.light{background:%s!important}.sq.dark{background:%s!important}"
        "@media(max-width:480px){.wrap{padding:4px}.soju img{width:56px;height:56px}}</style>"
    ) % (light, dark)
    js = (
        "<script>(function(){setTimeout(function(){var row=document.querySelector('.rowbtns');"
        "if(!row||document.getElementById('btnPgn'))return;"
        "var b=document.createElement('button');b.type='button';b.id='btnPgn';b.textContent='PGN';"
        "b.onclick=function(){var m=document.getElementById('moves');var t=m?m.innerText:'*';"
        "var p='[Event \"Meridium\"]\\n\\n'+t;if(navigator.clipboard)navigator.clipboard.writeText(p);"
        "else prompt('PGN',p);b.textContent='Copied!';setTimeout(function(){b.textContent='PGN'},1000);};"
        "row.appendChild(b);},200);})();</script>"
    )
    inj = css + js
    if "</body>" in html:
...
def apply_extra_features(code: str) -> str:
    if "Daily residual signal" not in code:
        home = (
            ...
            "                if _tu(st.session_state, \"Eric Rosen\"):\n"
            "                    st.toast(\"Unlocked: Eric Rosen!\"); _log.append(\"Unlocked Eric Rosen\")\n"
    ...
    if "Puzzle of the day" not in code:
        panel = (
            ...
            "                if _tu(st.session_state, \"Anna Cramling\"): st.toast(\"Unlocked: Anna Cramling!\")\n"
            "        with st.expander(\"Coach unlocks\", expanded=False):\n"
            "            _have = set(_uc(st.session_state))\n"
            "            for _n, _r in _UR.items(): st.write((\"OK\" if _n in _have else \"LOCKED\") + \" \" + _n + \" - \" + _r)\n"
            ...
            "                if st.session_state[\"_wins\"] >= 3 and _tu(st.session_state, \"Magnus\"): st.toast(\"Unlocked: Magnus!\")\n"
            "                if _gr(_user) >= 1100 and _tu(st.session_state, \"Fabi\"): st.toast(\"Unlocked: Fabi!\")\n"
            "            if _b.button(\"Mark PGN\", key=\"ul_pgn\"):\n"
            "                if _tu(st.session_state, \"Naroditsky\"): st.toast(\"Unlocked: Naroditsky!\")\n"
            "            if _c.button(\"Joined online\", key=\"ul_on\"):\n"
            "                if _tu(st.session_state, \"Botez\"): st.toast(\"Unlocked: Botez!\")\n"
            "        with st.expander(\"Custom coach\", expanded=False):\n"
            ...
            "        with st.expander(\"Results and rating\", expanded=False):\n"
