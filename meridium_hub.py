"""Meridium hub — Study + Languages; Gambits in Chess; Welcome popup."""
...
def _inject_after_page_config(code: str, blob: str) -> str:
...
def apply_learning_hub(code: str) -> str:
    ...
    if "meridium_learning_hub_v3" not in code:
        ...
        code = _inject_after_page_config(code, boot)
    ...
    if "welcome_to_meridium_v1" not in code:
        welcome = (
            ...
            "    except Exception:\n"
            "        st.info('Welcome to Meridium. You have entered the residual channel.')\n"
            "        if st.button('Enter Meridium', type='primary', key='welcome_enter_fallback'):\n"
            "            st.session_state.welcome_seen = True\n"
            "            st.rerun()\n"
        )
        code = _inject_after_page_config(code, welcome)
